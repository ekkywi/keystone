            ...
            <div class="space-y-8">
                ...
                <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                    ...
                    <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="col-span-1">
                            ...
                            <input class="w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 text-sm px-4 py-3 shadow-sm transition-all" name="name" required type="text" value="{{ old("name", $user->name) }}">
                            @error("name")
                        ...
                        <div class="col-span-1">
                            ...
                            <input class="w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 text-sm px-4 py-3 shadow-sm transition-all" name="email" required type="email" value="{{ old("email", $user->email) }}">
                            @error("email")
                                <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="col-span-1">
                            ...
                            <input class="w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 text-sm px-4 py-3 shadow-sm transition-all placeholder:text-slate-300" name="department" placeholder="e.g. Quality Assurance" type="text" value="{{ old("department", $user->department) }}">
                            @error("department")
                                <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>

                        {{-- Role (Adjusted to col-span-1) --}}
                        <div class="col-span-1">
                            ...
                            <div class="relative">
                                <select class="w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 text-sm px-4 py-3 shadow-sm transition-all appearance-none bg-white cursor-pointer" name="role">
                                    <option {{ old("role", $user->role) == "developer" ? "selected" : "" }} value="developer">Developer</option>
                                    <option {{ old("role", $user->role) == "quality_assurance" ? "selected" : "" }} value="quality_assurance">Quality Assurance</option>
                                    <option {{ old("role", $user->role) == "system_administrator" ? "selected" : "" }} value="system_administrator">System Administrator</option>
                                </select>
                ...
                <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                    <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex items-center gap-2">
                        ...
                        <div class="px-2 py-1 bg-amber-50 border border-amber-100 rounded text-[10px] text-amber-700 font-bold">Optional</div>
                    </div>
                    <div class="p-6">
                        <div class="mb-4 bg-amber-50 border border-amber-100 rounded-xl p-4 flex items-start gap-3">
                            ...
                            <div class="text-xs text-amber-800 leading-relaxed">
                                ...
                                Leave the fields below <strong>empty</strong> to keep current password.
                            </div>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="col-span-1">
                                <label class="block text-xs font-bold uppercase text-slate-500 mb-2">New Password</label>
                                <input class="w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 text-sm px-4 py-3 transition-all" name="password" placeholder="••••••••" type="password">
                                @error("password")
                                    <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            <div class="col-span-1">
                                ...
                                <input class="w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 text-sm px-4 py-3 transition-all" name="password_confirmation" placeholder="••••••••" type="password">
                            </div>
                        </div>
                    </div>
                </div>
